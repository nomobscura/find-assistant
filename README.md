...
The integration ships a Lovelace **strategy** — a small JS module that
builds the dashboard dynamically from whatever trackers currently
...
"Add Resource" step needed.** Just restart Home Assistant once after
installing/upgrading, then use the strategy in a dashboard's raw YAML
editor (**Edit Dashboard → ⋮ → Edit in YAML**) at **either** level —
pick one:
...
**One view within an otherwise normal dashboard:**
```yaml
views:
  - strategy:
      type: custom:find-assistant-trackers
